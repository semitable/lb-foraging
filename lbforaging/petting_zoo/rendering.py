...
class Viewer(object):
    ...
    def _draw_players(self, env):
        ...
        for agent in env.agents:
            ...
            sprite.color = env.agent_colors[agent]
            agents.append(sprite)
        for agent_sprite in agents:
            agent_sprite.update(scale=self.grid_size / agent_sprite.width)
        batch.draw()
        for agent in env.agents:
            self._draw_badge(*env.pos[agent], env.agent_levels[agent])
